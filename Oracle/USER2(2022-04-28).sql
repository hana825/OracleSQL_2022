...
/*
SELECT * : 모든 칼럼을 다 표시
SELECT st_num, st_name : 여러 칼럼 중에서 st_num, st_name 칼럼만 표시
...
SELECT st_num, st_dept, st_name, st_tel, st_addr, st_grade FROM tbl_student;
...
-- 내림차순 정렬
SELECT * FROM tbl_student ORDER BY st_name DESC;
...
-- 학과(st_dept)가 컴공과인 데이터만 SELECTION하여 이름(st_name)순으로 정렬하라
SELECT * FROM tbl_student WHERE st_dept = '컴공과' ORDER BY st_name;

-- 컴공과 학생들만 이름을 역순정렬하여 보여라
SELECT * FROM tbl_student WHERE st_dept = '컴공과' ORDER BY st_name DESC;

-- 전체 데이터를 학과명 순으로 정렬하고 학과명이 같은 데이터끼리는 이름순으로 정렬하라
SELECT * FROM tbl_student ORDER BY st_dept, st_name;

-- 전체 데이터의 개수
SELECT COUNT(st_dept) FROM tbl_student;

-- 학과별로 학생이 몇명인지 알고 싶다
SELECT st_dept, COUNT(st_dept) AS 학생수 -- 3 : 묶인 그룹에 포함된 데이터가 몇 개?
FROM tbl_student -- 1 : 데이터 가져오기
GROUP BY st_dept; -- 2 : st_dept(학과)가 같은 데이터까지 묶고

-- 데이터 양이 많을 경우 *를 사용하면 성능이 떨어진다.
SELECT COUNT(*) FROM tbl_student;

-- 전체 데이터 중에 컴공과 학생이 몇 명인가
SELECT COUNT(st_dept) FROM tbl_student WHERE st_dept = '컴공과';

/*
도구에서 데이터를 추가, 수정, 삭제 명령을 수행한 경우 Oracle DBMS에게 요청을 한 상태가 된다.
하지만 Oracle DBMS는 아직 스토리지의 DBF파일에 저장하지 않은 상태이다.
이 상태에서 도구를 종료해 버리면 Oracle DBMS는 어떤 문제가 발생하여 종료된 것으로 생각하고 데이터를 지워버린다.
도구를 사용하여 접속하였을 경우는 도구를 종료하기 전에 반드시 COMMIT을 해 주어야 한다.
*/
COMMIT;
...
/* 
데이터를 추가, 수정, 삭제하고 아직 COMMIT되지 않은 상태에서 추가, 수정, 삭제를 취소하는 명령
*/
ROLLBACK;
...
SELECT st_dept, COUNT(st_dept) FROM tbl_student GROUP BY st_dept ORDER BY COUNT(st_dept) DESC;

SELECT st_dept, COUNT(st_dept) AS 학생수 FROM tbl_student GROUP BY st_dept ORDER BY 학생수 DESC;

-- 학생수가 같으면 학과이름별로 정렬
SELECT st_dept, COUNT(st_dept) AS 학생수 FROM tbl_student GROUP BY st_dept ORDER BY 학생수 DESC, st_dept;
