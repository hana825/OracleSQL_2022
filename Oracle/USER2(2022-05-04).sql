...
/* 
tbl_score 테이블은 제1정규화가 완료되어 재설계되고 데이터를 import한 상태가 되었다.
*/
...
-- ROUND(AVG(sc_score),1) : AVG() 함수로 구한 결과를 소수점 2째에서 반올림하여 1째자리까지만 표현
...
-- 과목명(sc_subject칼럼)으로 그룹을 묶고 그룹내에서 총점과 평균을 계산하라
...
통계함수로 묶지 않은 칼럼은 반드시 GROUP BY에 명시해야 한다.
*/
...
WHERE절이나 HAVING절에 조건을 부여할 수 있다.

통계연산이 수행된 결과에 대하여 조건을 부여할 때는 
어쩔 수 없이 HAVING절에 조건을 부여해야 할 것이다.

하지만 연산결과가 아닌 어떤 조건에 일치하는 데이터들에게만 통계연산을 수행하고자 할 때는 
WHERE절에 조건을 부여할 수 없는지 반드시 고민해야 한다.

통계함수와 GROUP BY를 통한 통계연산은 SQL 조회명령에서 많이 느린 연산이다.
*/