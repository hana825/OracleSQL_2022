...
-- 드디어 일주일간 야근이다!
-- 경고 : UPDATE와 DELETE 명령은 WHERE이 없을 때 절대 실행 금지!!!!!!!!!!
UPDATE tbl_score
SET sc_sbcode = '001';
...
4. 새로 생성된 칼럼에 제약조건 설정
    ...
    NOT NULL 제약조건 : 값을 INSERT 할 때 이 칼럼은 비워 놓아서는 안된다.
*/
...
/* 
완전 JOIN
main table과 참조(foreign) table 간에 완전참조 무결성이 유지되는 경우에만 결과에 신뢰성이 유지된다.
만약 main table에는 데이터가 있는데 참조하는 값이 참조 table에 없는 경우, 결과는 main table의 데이터가 조회되지 않는다.
*/
...
-- 다수의 table에 저장된 데이터의 개수를 한 번에 확인하기
SELECT * FROM tbl_score, tbl_subject;
