...
1. TableSpace 생성
    ...
    System TableSpace에는 중요한 데이터들이 저장되므로 가급적 일반적인, 대량의 Data를 저장하지 않는 것이 좋다.
    
    때문에 프로젝트별로 TableSpace를 만드는 것이 좋다.
*/

CREATE TABLESPACE firstDB
DATAFILE 'C:/oraclexe/app/data/first.dbf'
...
/* 
사용자를 생성하면서 DEFAULT TABLESPACE를 연결하지 않았을 경우 생성된 사용자 정보에 TABLESPACE 추가하기
...
##DML(Data Manipulation Language, Data Management Language : 데이터 조작어, 관리어)
...
손상된 data가 저장된 DB 시스템은 '무결성이 파괴'되었다라고 표현한다.

"여기서는 학습의 편의성을 위하여 사용자에게 최고권한을 부여할 것이다."
