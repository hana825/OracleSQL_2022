...
-- 이 Table에 INSERT를 수행할 때는 데이터 검증이 잘 되어 잘못된 데이터가 추가되는 것을 방지할 수 있다.
-- 하지만 UPDATE, DELETE를 수행할 때는 복수의 칼럼으로 조건절을 수행해야 한다.
-- 이때 칼럼을 누락시키거나, 잘못된 값으로 조건을 수행하여 데이터 무결성이 파괴될 수 있다.

-- 이럴 때는 실제 데이터와 관련없는 단일 칼럼을 추가하고 PK로 설정하는 설계를 할 필요가 있다.
...
-- 전체 칼럼에 데이터를 포함하여 INSERT를 할 때는 칼럼 리스트를 생략할 수 있다.
